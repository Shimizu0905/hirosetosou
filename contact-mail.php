...
$subject = "【お見積もり・お問い合わせ】広瀬塗装店 ホームページからのお問い合わせ";
...
$require = array('inquiry_type', 'construction_area', 'name', 'email', 'address', 'privacy'); // 必須にしたい項目
...
$refrom_name = "株式会社広瀬塗装店";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "【自動返信】お見積もり・お問い合わせを受け付けました - 株式会社広瀬塗装店";
...
この度は、株式会社広瀬塗装店にお見積もり・お問い合わせいただき、誠にありがとうございます。

お問い合わせ内容を確認いたしました。
担当者より2営業日以内にご返信いたします。

お急ぎの場合は、お電話（0120-40-1616）にてお問い合わせください。
...
?>
	...
	<head>
		...
		<title>お見積もり・お問い合わせ確認 - 株式会社広瀬塗装店</title>
		<link rel="stylesheet" href="./style.css">
		<script src="./main.js" defer></script>
		<!-- Google tag (gtag.js) -->
		<script async src="https://www.googletagmanager.com/gtag/js?id=G-Y9GG9WQPWH"></script>
		<script>
	...
	<body>
		...
		<main class="contact-done-page">
			<section class="contact-done">
				<div class="contact-done__inner inner">
					<h1 class="section-title text-outline-white section-title-penki">お見積もり・お問い合わせ確認</h1>
					<p class="contact-done__text">以下の内容で間違いがなければ、「送信する」ボタンを押してください。</p>

					<?php if ($empty_flag == 1) { ?>
						<div class="contact-done__error">
							<h4>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4>
							<?php echo $errm; ?>
							<br><br>
							<button type="button" class="btn-confirm btn-confirm--back" onclick="history.back()">
								<span class="btn-confirm__arrow btn-confirm__arrow--left"></span>
								<span class="btn-confirm__text">前画面に戻る</span>
							</button>
						</div>
					<?php } else { ?>
						<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST" class="contact-done__form">
							<div class="contact-done__form-container">
								<?php echo confirmOutput($_POST); //入力内容を表示
								?>
							</div>
							<div class="contact-done__form-submit">
								...
								<button type="submit" class="btn-confirm btn-confirm--submit">
									<span class="btn-confirm__text">送信する</span>
									<span class="btn-confirm__arrow"></span>
								</button>
							</div>
						</form>
					<?php } ?>
...
<?php
		...
		function postToMail($arr)
		{
			...
			// フィールド名の日本語マッピング
			$fieldNames = array(
				'inquiry_type' => 'お問い合わせ内容の選択',
				'construction_area' => '工事希望箇所',
				'name' => 'お名前',
				'furigana' => 'ふりがな',
				'email' => 'メールアドレス',
				'phone' => 'お電話番号',
		...
		function confirmOutput($arr)
		{
			...
			foreach ($arr as $key => $val) {
				...
				if ($key === 'privacy') {
					...
					$html .= '<div class="contact-done__form-item">';
					$html .= '<div class="contact-done__form-label">' . h($displayKey) . '</div>';
					$html .= '<div class="contact-done__form-value">' . h($out);
				...
				// contact-done用のHTML形式に変更
				$html .= '<div class="contact-done__form-item">';
				$html .= '<div class="contact-done__form-label">' . $displayKey . '</div>';
				$html .= '<div class="contact-done__form-value">' . $out;
